...
var AwsSecretsStrategy = class {
  ...
  async getSecret(name) {
    try {
      const result = await withRetry(() => this.client.send(new import_client_secrets_manager.GetSecretValueCommand({ SecretId: name })), this.retryConfig);
      return {
...
var AzureSecretsStrategy = class {
  ...
  constructor(config) {
    ...
    const credential = config.config?.["credential"] ?? new import_identity.DefaultAzureCredential();
